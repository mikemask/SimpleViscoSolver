void creep(int model, int n_part, int n_bond, Part *vec_part, Bond *vec_bond, double kn, double ks, double mun, double mus)
{
    double F_ext[3], t_tot, n_double, t_out, n_out_double, dt, t_stop, n_stop_double;
    int n, n_out, n_stop, count=0;

    std::cout << "Insert force vector applied to top layer of particles [m]:  ";
    std::cin >> F_ext[0] >> F_ext[1] >> F_ext[2];

    std::cout << "Insert total time of simulation [s]: ";
    std::cin >> t_tot;

    std::cout << "Insert time at which force is removed [s]: ";
    std::cin >> t_stop;
    ...
    n_stop_double = t_stop/dt;
    n_stop = (int) n_stop_double;

    std::ofstream out_file;

    if (model==0)
        out_file.open("creep_undamped.csv");

    else if (model==1)
        out_file.open("creep_kelvin.csv");

    else
        out_file.open("creep_maxwell.csv");
